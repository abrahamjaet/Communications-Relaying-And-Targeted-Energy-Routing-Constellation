...
## Function to save design class objects ##
...
## Function to obtain every possible matching of orbits to number of sats (stars & bars problem)
# https://stackoverflow.com/questions/28965734/general-bars-and-stars
def partitions(n, k):
    for c in itertools.combinations(range(n+k-1), k-1):
        yield [b-a-1 for a, b in zip((-1,)+c, c+(n+k-1,))]

## Functions to allow for multiprocessing of each model ##
...
## Function to call drivers (model) functions in parallel ##
def driverCall(design):
    ...
    filename = 'allDesignsBackup/design' + str(design.ID) + '.dat'
...
def main():
    start = time.time()
    # Import orbitDict files (list of all trajectories used in analysis)
    ...
    ## Smaller sample size of design variables ##
    diameterTxM = 1 # [m] antenna diameter of the receiver on the moon
    ...
    # Coverage Figure of Merit Calculation in parallel
    print("Calculate FOM")
    pFOM = multiprocessing.Pool(os.cpu_count()-1)
    orbitDesign_temp = pFOM.map(comms.FOM, orbitDesign)
    pFOM.close()
    pFOM.join()
    orbitDesign = orbitDesign_temp
    ...
    # Run each design through driverCall function (models) in parallel 
    currTime = str(time.time() - start)
    ...
    ## Function to save list of scored final designs ##
